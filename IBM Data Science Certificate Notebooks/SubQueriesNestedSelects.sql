...
WHERE SALARY < AVG(SALARY);
	--This function gives an error denoting the misuse of an aggregate function.
...
SELECT EMP_ID, SALARY, MAX(SALARY) AS MAX_SALARY FROM EMPLOYEES;
	--Shows an error saying that no group by clause is specified.
...
SELECT * FROM (SELECT EMP_ID, F_NAME, L_NAME, JOB_ID, DEP_ID FROM EMPLOYEES) AS EMPPUBLIC;